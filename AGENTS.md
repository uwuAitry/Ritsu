...
src/
  ...
  atmos/            Three.js 空间摆位视图（房间形状 / 辉光轨迹 / 房间粒子）
...
- 摆位视图的一切动画都必须是 `timeMs` 的纯函数（禁止滚动缓冲 / 墙钟 / 帧计数），房间粒子用固定 seed：预览与离线逐帧导出必须得到同一画面。取景按房间轮廓收紧（不再按外接球保守留白）；对象小球保持固定世界半径（上一版观感），而粒子尺寸与拖尾按取景距离换算屏幕尺寸。
- 摆位视图不含后处理泛光：辉光由加色 sprite 表达；房间线、听者环、粒子一律压低亮度，只作参照物。
